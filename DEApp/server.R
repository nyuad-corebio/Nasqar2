...
shinyServer(function(input, output, session) {
    ...
    dataObs <- reactiveValues(
        orgCount = read.delim(paste(getwd(), "www/exampleData/pnas-count_singleFactor.txt", sep = "/"), header = T, row.names = 1),
        orgMeta = read.delim(paste(getwd(), "/www/exampleData/pnas-count_singleFactor-meta.txt", sep = ""), header = T)
    )

    observeEvent(input$MultiSubmit, {
        if (is.null(input$countFileMulti) & is.null(input$metaTabMulti)) {
            dataObs$orgCount <- read.delim(paste(getwd(), "www/exampleData/ReadCounts-Chen-edgeRSpringer-multiFactor.csv", sep = "/"), header = T, sep = input$coutFileSepMulti, row.names = 1)
            dataObs$orgMeta <- read.delim(paste(getwd(), "/www/exampleData/ReadCounts-Chen-edgeRSpringer-multiFactor-meta.csv", sep = ""), header = T, sep = input$metaSepMulti)
    ...
    observeEvent(input$dataSubmit, {
        if (is.null(input$countFile) & is.null(input$metaTab)) {
            dataObs$orgCount <- read.delim(paste(getwd(), "www/exampleData/pnas-count_singleFactor.txt", sep = "/"), header = T, sep = input$coutFileSep, row.names = 1)
            dataObs$orgMeta <- read.delim(paste(getwd(), "/www/exampleData/pnas-count_singleFactor-meta.txt", sep = ""), header = T, sep = input$metaSep)
    ...
    output$overallDataSummary <- renderTable(
        {
            if (input$dataSubmit) {
                isolate({
                    ...
                    if (is.null(meta) & !is.null(count)) {
                        stop("Please provide the corresponding input 2: Meta-data Table!")
                    } else if (!is.null(meta) & is.null(count)) {
                        stop("Please provide the input 1: Raw Count Data!")
                    } else if (!is.null(meta) & !is.null(count)) {
                        if (length(grep("Error", count[1])) == 1) {
                            ...
                            if (dim(orgCount)[2] != dim(orgMeta)[1]) {
                                stop("Input files do not correspond with each other. ")
                            }
                            # else if ( sum(colnames(orgCount) == orgMeta[,1]) !=dim(orgMeta)[1] ) {stop("Input files do not correspond with each other. ")}
                            else if (dim(orgMeta)[2] < 2) {
                                stop("Input 2 file format is wrong.")
                            }
                        }
                    }
                ...
                isolate({
                    ...
                    if (is.null(meta) & !is.null(count)) {
                        ...
                        if (length(grep("Error", count[1])) == 1) {
                            ...
                            if (dim(orgCount)[2] != dim(orgMeta)[1]) {
                                stop("Input files do not correspond with each other. ")
                            } else if (sum(colnames(orgCount) == orgMeta[, 1]) != dim(orgMeta)[1]) {
                                stop("Input files do not correspond with each other. ")
                            } else if (dim(orgMeta)[2] < 3) {
                                stop("Input 2 file format is wrong.")
                            }
    ...
    output$edgerRes <- DT::renderDataTable(
        ...
        options = list(
            order = list(2, "asc"), pageLength = 20,
    ...
    output$edgerDownload <- downloadHandler(
        filename = function() {
            paste("edger-DEG-res-", Sys.Date(), ".txt", sep = "")
        },
    ...
    output$voomDownload <- downloadHandler(
        filename = function() {
            paste("voom-DEG-res-", Sys.Date(), ".txt", sep = "")
        },
        content = function(file) {
            write.table(
                rbind(
    ...
    output$deseq2Download <- downloadHandler(
        filename = function() {
            paste("DESeq2-DEG-res-", Sys.Date(), ".txt", sep = "")
        },
        content = function(file) {
            write.table(
                rbind(
        ...
        if (input$decompAnalysis) {
            isolate({
                ...
                if (as.character("edger") %in% input$decompMethods & as.character("voom") %in% input$decompMethods & as.character("deseq2") %in% input$decompMethods) {
                    ...
                    vennDiagram(venncount,
                        names = c("edgeR", "voom", "DESeq2"),
                    ...
                    vennDiagram(cbind(edgerDecomp()$table$filter, voomDecomp()$filter),
                        include = c("both"),
                    ...
                    vennDiagram(cbind(edgerDecomp()$table$filter, deseq2Decomp()$filter),
                        include = c("both"),
                    ...
                    vennDiagram(cbind(voomDecomp()$filter, deseq2Decomp()$filter),
                        include = c("both"),
            ...
            isolate({
                ...
                if (input$decompP == "normp") {
                    title <- paste("Below results are based on the nominal p ",
                        ...
                        " and FC = ", as.character(input$decompfc),
                        sep = ""
                    )
                }
    ...
    output$overlap_genes_download <- downloadHandler(
        filename = function() {
            paste("comparison_results_", paste(input$decompMethods, collapse = "-"), "_", Sys.Date(), ".zip", sep = "")
        },
        content = function(fname) {
            ...
            if (as.character("edger") %in% input$decompMethods & as.character("voom") %in% input$decompMethods & as.character("deseq2") %in% input$decompMethods) {
                ...
                ol1 <- cbind(
                    edgerRes[match(attr(vennres, "intersections")$`voom:edgeR:DESeq2`, rownames(edgerRes)), ],
                ...
                ol2 <- cbind(
                    edgerRes[match(attr(vennres, "intersections")$`voom:edgeR`, rownames(edgerRes)), ],
                    voomRes[match(attr(vennres, "intersections")$`voom:edgeR`, rownames(voomRes)), ]
                )
                ol3 <- cbind(
                    edgerRes[match(attr(vennres, "intersections")$`edgeR:DESeq2`, rownames(edgerRes)), ],
                    deseq2Res[match(attr(vennres, "intersections")$`edgeR:DESeq2`, rownames(deseq2Res)), ]
                )
                ol4 <- cbind(
                    voomRes[match(attr(vennres, "intersections")$`voom:DESeq2`, rownames(voomRes)), ],
                ...
                ol1 <- cbind(
                    edgerRes[match(attr(vennres, "intersections")$`DESeq2:edgeR`, rownames(edgerRes)), ],
    ...
    output$step2Tostep3 <- renderUI({
        if (input$rmlow) {
            isolate({
                list(
                    actionButton("switchTab21", label = "GO TO STEP 3: DE Analysis - edgeR", class = "btn-info"),
    ...
    observeEvent(input$switchTab01, {
        newtab <- switch(input$menu1,
            "intro" = "dataInputSingle",
            "dataInputSingle" = "intro"
        )
        updateTabItems(session, "menu1", newtab)
    })
    observeEvent(input$switchTab02, {
        newtab <- switch(input$menu1,
            "intro" = "dataInputMulti",
            "dataInputMulti" = "intro"
        )
        updateTabItems(session, "menu1", newtab)
    })
    ...
    observeEvent(input$switchTab11, {
        newtab <- switch(input$menu1,
            "dataInputSingle" = "dataSummary",
            "dataInputSingle" = "dataInputMulti"
        )
        updateTabItems(session, "menu1", newtab)
    })
    ...
    observeEvent(input$switchTab12, {
        newtab <- switch(input$menu1,
            "dataInputMulti" = "dataSummary",
            "dataSummary" = "dataInputMulti"
        )
        updateTabItems(session, "menu1", newtab)
    })
    ...
    observeEvent(input$switchTab22, {
        newtab <- switch(input$menu1,
            "dataSummary" = "limmavoom",
            "limmavoom" = "dataSummary"
        )
        updateTabItems(session, "menu1", newtab)
    })
