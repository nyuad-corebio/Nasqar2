...
tabPanel(
    ...
    fluidRow(
        column(
            4, wellPanel(
                conditionalPanel(
                    "input.analysisres_tabset=='Volcano Plot'",
                    selectizeInput("analysisres_test",
                        label = "Select Test for Volcano Plot",
                        choices = NULL
                    ),
                ...
                conditionalPanel(
                    "input.analysisres_tabset=='Scatterplot of Fold Changes'",
                    selectizeInput("analysisres_groups",
                        label = "Select Groups for Scatterplot",
                        choices = NULL,
                ...
                conditionalPanel(
                    "input.analysisres_tabset=='Volcano Interactive MA Plot (DESeq)'",
                    selectizeInput("analysisres_test_ma", label = "Select Test for Volcano Plot", choices = NULL),
                    sliderInput("alpha", "Adjusted p-value treshold", min = 0, max = 1, value = 0.1, step = 0.001),
                    numericInput("analysisres_ma_plot_ylim",
        ...
        column(
            ...
            tabsetPanel(
                ...
                tabPanel(
                    title = "Scatterplot of Fold Changes",
                    # h5(textOutput("corPR")),
                ...
                tabPanel(
                    title = "Volcano Interactive MA Plot (DESeq)",
                    plotOutput("plotma", click = "plotma_click", width = 500, height = 500),
                    plotOutput("plotcounts", width = 500, height = 500)
                )
            ) # tabsetPanel
        ) # column
    ) # fluidrow
