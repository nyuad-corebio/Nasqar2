...
if (FALSE) {
    ...
    dotplot_fun(
        data_long = data_long, geneids = geneids,
        genelabel = "MGIsymbol",
...
dotplot_fun <- function(data_long,
    ...
    if ((ll == 0) || (length(sel_group) == 0)) {
        ...
        p <- ggplot(subdat_all, aes(x = group, y = y, fill = group)) +
            geom_boxplot()
        ...
        if (max(data_long[, ytype]) >= 500) {
            p <- p + scale_y_continuous(trans = log2_trans(), breaks = 2^(0:100))
        }
