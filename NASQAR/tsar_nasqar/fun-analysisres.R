...
rna_volcanoplot <- function(data_results, geneids = NULL,
    ...
    res$color[which((abs(res$logFC) < absFCcut) * (res$adj.P.Val < fdrcut) == 1)] <- paste0(pvalname, "<", fdrcut, " & abs(logfc)<", absFCcut)
    # res$color[which((abs(res$logFC)>absFCcut)*(res$adj.P.Val>fdrcut)==1)] = paste0(pvalname,">",fdrcut, " & abs(logfc)>",absFCcut)
    res$color <- factor(res$color, levels = unique(c(
        "None",
    ...
    if (length(levels(res$color)) > 3) {
        p <- p + scale_color_manual(
            values = c("grey40", "grey60", "green3", "grey70", "red2"),
            limits = levels(res$color),
            name = "Significance"
        )
    } else {
        p <- p + scale_color_manual(
            values = c("grey40", "green3", "red2"),
            limits = levels(res$color),
            name = "Significance"
        )
    }
...
rna_volcanoplot_ggvis <- function(data_results, geneids = NULL,
    ...
    res %>%
        ggvis(~logFC, ~ -log10(P.Value), fill = ~color, key := ~id) %>%
        layer_points() %>%
        add_axis("x", title = "Log2(FC)") %>%
        add_axis("x",
            orient = "top", title = paste0("Comparison: ", unique(res$test)),
            ticks = 0
        ) %>%
        add_axis("y", title = "-log10(p-value)") %>%
        add_tooltip(all_values, "hover") %>%
        add_legend("fill", title = "Significance")
}
...
rna_scatterplot <- function(data_long, geneids = NULL, group_sel = NULL,
    ...
    p <- ggplot(pp_wide, aes(
        x = g1, y = g2,
        color = factor(color), text = unique_id
    )) +
        geom_point()
